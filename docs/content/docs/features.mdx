...
<Steps>
  <Step>
    ...
    `APX_UV_PATH` or `APX_BUN_PATH` — use a custom binary path.
  </Step>
  <Step>
    ### System PATH
    Uses the existing installation if found on your system.
  </Step>
  <Step>
    ### `~/.apx/bin/`
    Checks for a previously downloaded binary with matching version.
  </Step>
  <Step>
    ### Download
    Fetches the pinned version from GitHub releases to `~/.apx/bin/`.
  </Step>
</Steps>
...
| `Dependency.Session`    | `Session`         | Database session (lakebase addon only)          | `session: Dependency.Session`    |
