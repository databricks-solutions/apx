...
<Callout type="info">
  All assistant addons automatically install apx skill files (quick reference + backend/frontend patterns)
  during `apx init`. Each addon uses its configured skill path — most use `.claude/skills/apx/`,
  while Codex uses `.agents/skills/apx/`.
</Callout>
...
| `list_registry_components`   | List all available components in a registry                       |
