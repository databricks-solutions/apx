...
mod tests {
    ...
    fn test_embed_single_text() {
        ...
        let text = "button component for user interface";
        let embedding = embedder.embed(text).expect("Failed to generate embedding");

        assert_eq!(embedding.len(), EMBEDDING_DIM, "Embedding should have correct dimension");
    ...
    fn test_embed_batch() {
        ...
        let texts = vec![
            ...
            "dialog modal window".to_string(),
            "input form field".to_string(),
        ];

        let embeddings = embedder.embed_batch(&texts).expect("Failed to generate embeddings");

        assert_eq!(embeddings.len(), texts.len(), "Should generate one embedding per text");
        for embedding in embeddings {
            assert_eq!(embedding.len(), EMBEDDING_DIM, "Each embedding should have correct dimension");
        }
    }
}
