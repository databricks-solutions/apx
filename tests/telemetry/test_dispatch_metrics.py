"""Verify all 6 APX dispatch pipeline metrics are collected when APX_PERF=1.
...
6 histogram metrics must appear in the OTEL collector output.
"""
...
class TestDispatchMetrics:
    ...
    def test_dispatch_metrics_unit_is_microseconds(
        ...
        """APX dispatch metrics must report in microseconds."""
        for _, m in flat_metrics_with_scope(otel_collector):
            if m.name in APX_DISPATCH_METRICS:
                assert m.unit == "us", (
