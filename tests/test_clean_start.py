...
async def test_dev_start_after_clean(tmp_path: Path) -> None:
    ...
    assert not metadata_path.exists(), "_metadata.py should be removed by git clean -fdx"
    ...
    try:
        ...
        assert backend_response.json().get("version") is not None, "Should return version"
...
async def test_build_after_clean(tmp_path: Path) -> None:
    ...
    assert len(wheel_files) == 1, f"Should have exactly one wheel file, found: {wheel_files}"
    ...
    with zipfile.ZipFile(wheel_path, 'r') as wheel:
        ...
        assert len(metadata_files) > 0, f"_metadata.py should be in wheel. Contents: {wheel_contents}"
        ...
        assert "api_prefix" in metadata_content, "_metadata.py should contain api_prefix"
        ...
        assert len(dist_files) > 0, f"__dist__ should have files in wheel. Contents: {wheel_contents}"
    ...
    assert wheel_path.name in requirements_content, "requirements.txt should reference the wheel"
...
async def test_build_produces_complete_wheel(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheel_path, 'r') as wheel:
        ...
        for pattern in required_patterns:
            ...
            assert len(matches) > 0, f"Pattern '{pattern}' not found in wheel. Contents: {wheel_contents[:20]}..."
        
        # __dist__ should have actual UI build artifacts
        dist_files = [f for f in wheel_contents if "__dist__/" in f and not f.endswith("__dist__/")]
        assert len(dist_files) > 0, f"__dist__ should contain UI build files. Found: {dist_files}"
        
        # Should have index.html in __dist__
        index_files = [f for f in dist_files if "index.html" in f]
        assert len(index_files) > 0, f"__dist__ should contain index.html. Dist files: {dist_files}"
