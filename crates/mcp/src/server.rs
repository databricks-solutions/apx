...
use apx_core::components::{SharedCacheState, needs_registry_refresh, sync_registry_indexes};
...
async fn search_registry_components_tool(
    ...
    use apx_core::common::read_project_metadata;
    use apx_core::components::UiConfig;
...
async fn add_component_tool(ctx: Arc<AppContext>, args: AddComponentArgs) -> ToolResult {
    ...
    match add_components(&ctx.app_dir, &[input], args.force).await {
        Ok(_result) => {
